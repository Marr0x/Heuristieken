...
def gnomeSort(genes):
	...
	while i < n:
		if i and genes[i] < genes[i - 3]:
			genes[i], genes[i - 3] = genes[i - 3], genes[i]
			genes[i - 1], genes[i - 2] = genes[i - 2], genes[i - 1]
			i -= 3
		elif i and genes[i] < genes[i - 2]:
			genes[i], genes[i - 2] = genes[i - 2], genes[i]
			i -= 2
		elif i and genes[i] < genes[i - 1]:
